...
# ---------------------------------------------------------------------------
# CONFIG
# API_KEY: leave as-is. It's read from the MERAKI_DASHBOARD_API_KEY environment
#          variable (set via a GitHub secret when run through Actions, or a
#          local env var when run manually). Never hardcode a real key here.
# ORG_ID:  optional fallback only. If you're using the --org-id command-line
#          flag (recommended - this is what the .yml workflow template uses),
#          you can leave this blank ("") and ignore it. This same .py file can
#          then be reused unmodified across every org's repo.
# ---------------------------------------------------------------------------
API_KEY = os.environ.get("MERAKI_DASHBOARD_API_KEY", "PASTE_YOUR_MERAKI_API_KEY_HERE")
ORG_ID = ""   # optional fallback - normally leave blank and use --org-id instead
...
# ---------------------------------------------------------------------------
def main():
    parser = argparse.ArgumentParser(description="Meraki Organization Network Health Report")
    parser.add_argument("--org-id", dest="org_id", default=None,
                         help="Meraki organization ID to run against. Overrides the ORG_ID variable in this file if given.")
    args = parser.parse_args()

    if not API_KEY or API_KEY == "PASTE_YOUR_MERAKI_API_KEY_HERE":
        log("ERROR: Set the MERAKI_DASHBOARD_API_KEY environment variable (or edit API_KEY near the top of this script) with your real Meraki API key.")
        sys.exit(1)
    ...
    # Priority: --org-id command-line flag > ORG_ID variable near the top of this file.
    # This is what lets the SAME script be reused across repos/orgs by just
    # changing the --org-id passed in from the .yml workflow, with no need
    # to edit this .py file per org.
    org_id_to_use = args.org_id or (ORG_ID if ORG_ID and ORG_ID != "PASTE_YOUR_ORG_ID_HERE" else None)
